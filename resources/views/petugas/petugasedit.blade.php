...
<body class="form-page">
    <div class="container p-5" style="max-width: 600px;">
        ...
        <h1 class="mb-2 items-center">Edit Data Petugas</h1>
        <p class="form-description">Perbarui data petugas sesuai kebutuhan</p>
        <div class="alert alert-info info-badge mb-4">
            📋 Mengedit data petugas: <strong>{{ $datapetugas->nama_petugas }}</strong>
        </div>

        <form action="{{ route('petugas.update', $datapetugas->id_petugas) }}" method="post">
            @csrf
            @method('PUT')

            <div class="mb-3">
                <label for="nip" class="form-label"><strong>NIP</strong></label>
                <input type="text" class="form-control" id="nip" name="nip" value="{{ $datapetugas->nip }}" required>
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control" id="no_hp_petugas" name="no_hp_petugas" value="{{ $datapetugas->no_hp_petugas }}" required>
            </div>
